...
## 2. Migrations — apply in order, `0007` through `0012`
...
between files (`0012` alters columns `0009`/`0010` create):

```bash
cd worker
bun run migrate:local   # dry-run against local D1 first — verify no errors
bun run migrate         # wrangler d1 migrations apply vellum (applies every unapplied 0001..0012 migration)
```
...
| `0012_echo_guards.sql` | Phase 17/18 post-review (this pass) — partial `UNIQUE INDEX` on `echo_events` for `sinking` dedup; `rooms.fading_echoed_at`; `warmth_state.{checked_score, warmed_echoed_at}` |
...
# The /s/<slug> router prefix works even for the default surface's own name
curl -s "$HOST/s/vellum/api/state" | python3 -m json.tool | head -3   # expect the SAME shape as GET /api/state
```
...
- **D1 migrations**: no down-migrations exist (§2). If `0007`–`0012` need to be undone after a bad
